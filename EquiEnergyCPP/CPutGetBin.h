...
public: 
	CPutGetBin(const string & _id=string(), unsigned int _nDumpFile=0, size_t _capacity=0, string _grandPrefix=string(), unsigned int _suffix=0); 
	CPutGetBin(const CPutGetBin &); 
	CPutGetBin & operator=(const CPutGetBin &); 
	~CPutGetBin();

	void SetBinID(string _id, unsigned int _suffix=0) { id = _id; suffix=_suffix; }
