...
//#define VERBOSE_WARNINGS    //Display warnings.	// commented out by HW
//#define VERBOSE_DETOUTPUT   //Display detailed output. // commented out by HW
#define STRLEN 192

double dw_GRADSTPS_CSMINWEL = 1.0e-04;  //Default value.  Will be overwritten by the data in the input file if it exists.
...
//      f1=f; f2=f; f3=f; retcode2=retcode1; retcode3=retcode1;
      ...
      #ifdef DW_CSMINWELL_PRINTOUT
      dw_csminwel_SetPrintFile(NULL); // added by HW
...
#undef TERMINATEVALUE
   ...
   static int numgrad(double *g, double *x, int n,
      ...
      for (i=0, xp=x; i<n; i++, xp++, g++) {
         delta=SCALE*dw_GRADSTPS_CSMINWEL, deltai=1.0/delta;  //e+5/SCALE;
   ...
   static int numgrad(double *g, double *x, int n,
      ...
      badg = 0;
      for (i=0, xp=x; i<n; i++, xp++, g++) {
         dh = fabs(*xp)<=1 ? dw_GRADSTPS_CSMINWEL : dw_GRADSTPS_CSMINWEL*(*xp);
