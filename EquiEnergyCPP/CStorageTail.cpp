...
TStorage::TStorage(int NumberParameters, const string &StorageDirectory, const string &RunId, int Node) 
  : n_parameters(NumberParameters), size(NumberParameters+5), storage_directory(StorageDirectory), run_id(RunId), node(Node), 
...
void TStorage::CumulativeImportanceWeights(double lambda, const TDenseVector &log_prior, const TDenseVector &log_likelihood, const TDenseVector &log_density)
{
  ...
  weights.Resize(log_density.dim);
  weights.vector[0]=exp(LOG_WEIGHTS(log_prior.vector[0],log_likelihood.vector[0],log_density.vector[0],lambda) - sum);
  for (int i=1; i < log_density.dim; i++)
    weights.vector[i]=weights.vector[i-1] + exp(LOG_WEIGHTS(log_prior.vector[i],log_likelihood.vector[i],log_density.vector[i],lambda) - sum);
}
...
/*
  ...
  by the log density.  The original files are deleted.

  Creates info file consisting of the stage, the vector of log priors, the vector
  of log likelihoods, and the vector of log densities.
...
void TStorage::FinalizeMasterNode(int stage)
{
  ...
  // write draws file                           
  draws.SortRows(n_parameters+2);     
  OpenFile(file,true,"draws",stage);
  draws.WriteBinary(file);
  file.close();

  // write info file
  TDenseVector log_prior=draws.ColumnVector(n_parameters), log_likelihood=draws.ColumnVector(n_parameters+1), log_density=draws.ColumnVector(n_parameters+2);
  OpenFile(file,true,"info",stage);
  file.write((char*)(&stage),sizeof(int));
  log_prior.WriteBinary(file);
  log_likelihood.WriteBinary(file);
  log_density.WriteBinary(file);
  file.close();
}

void TStorage::ReadInfo(int stage, TDenseVector &log_prior, TDenseVector &log_likelihood, TDenseVector &log_density)
{
  fstream file;
  ...
  log_prior.ReadBinary(file);
  log_likelihood.ReadBinary(file);
  ...
  if (stage > 0)
    {
      ...
      ReadInfo(stage-1,log_prior,log_likelihood,log_density);
      CumulativeImportanceWeights(lambda,log_prior,log_likelihood,log_density);
      ...
      for (int m=1+(log_density.dim)/number_striations, hi=log_density.dim, i=n_striations-1; i >= 0; i--)
	{
	  if (hi == 0)
	    {
	  ...
	  int j = (hi > m) ? hi-m : 0;
	  for ( ; (j > 0) && (log_density.vector[j] == log_density.vector[j-1]); j--);
	...
	striation_boundary[i]=log_density.vector[striation_offset[i+1]];
...
void TStorage::EqualWeightedDraw(TDraw &x, int striation)
{
  ...
  x.Set(in[striation].RowVector(idx,0,n_parameters-1), in[striation](idx,n_parameters),in[striation](idx,n_parameters+1), 
	in[striation](idx,n_parameters+2), in[striation](idx,n_parameters+3), in_weighted(idx_in_weighted,n_parameters+4));
}
...
void TStorage::ImportanceWeightedDraw(TDraw &x)
{
  ...
  x.Set(in_weighted.RowVector(idx_in_weighted,0,n_parameters-1), in_weighted(idx_in_weighted,n_parameters), in_weighted(idx_in_weighted,n_parameters+1), 
	in_weighted(idx_in_weighted,n_parameters+2), in_weighted(idx_in_weighted,n_parameters+3), in_weighted(idx_in_weighted,n_parameters+4));
}
...
void TStorage::AddDraw(const TDraw &x)
{
  ...
  out(idx_out,n_parameters)=x.LogPrior();
  out(idx_out,n_parameters+1)=x.LogLikelihood();
  out(idx_out,n_parameters+2)=x.LogDensity();
  out(idx_out,n_parameters+3)=x.Group();
  out(idx_out,n_parameters+4)=x.Index();
  idx_out++;
}
...
int TStorage::Striation(const TDraw &x) 
{
  return StrictLeastUpperBound(striation_boundary, x.LogDensity());
}
...
void TStorage::WriteASCII_draws(int stage)
{
  fstream file_in, file_out;
  OpenFile(file_in,false,"draws",stage);
  OpenFile(file_out,true,"draws_ascii",stage);
  file_out << setprecision(12) << scientific;

  int rows;
  file_in.read((char*)(&rows),sizeof(int));
  file_in.seekg(2*sizeof(int)+sizeof(bool),ios::beg);
  TDenseVector x(size);
  for (int i=0; i < rows; i++)
    {
      x.UniqueMemory();
      file_in.read((char*)(x.vector),size*sizeof(double));
      file_out << x;
    }
...
void TStorage::WriteASCII_info(int stage)
{
  TDenseVector log_prior, log_likelihood, log_density;
  ReadInfo(stage,log_prior,log_likelihood,log_density);
  fstream file_out;
  OpenFile(file_out,true,"info_ascii",stage);
  file_out << setprecision(12) << scientific << log_prior << log_likelihood << log_density;
  file_out.close();
}
/////////////////////////////////////////////////////////////////////////////////
/// End TStorage
...
// These routines can be called after a call to Setup() to check the class
void TStorage::ErrorClass(void)
{
  WriteASCII_info(stage_out-1);
  fstream file_out;
  OpenFile(file_out,true,"error_info_weights",stage_out);
  file_out << setprecision(12) << scientific << weights;
  file_out.close();
  OpenFile(file_out,true,"error_info_striations",stage_out);
  file_out << setprecision(12) << scientific << "Offset:" << endl;
  for (int i=0; i < (int)striation_offset.size(); i++) file_out << "  " << striation_offset[i];
  file_out << endl << "Width:" << endl;
  for (int i=0; i < (int)striation_width.size(); i++) file_out << "  " << striation_width[i];
  file_out << endl << "Boundary:" << endl << striation_boundary;
  abort();
}
  ...
  for (int i=0; i < rows; i++)
    {
      ...
      x.Set(z.SubVector(0,n_parameters-1), z(n_parameters), z(n_parameters+1), z(n_parameters+2), z(n_parameters+3), z(n_parameters+3));
      s=Striation(x);
